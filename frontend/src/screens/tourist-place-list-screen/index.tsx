...
import { TouristPlaceSummary, useTouristPlaces } from '@/services/tourist-place';
...
export default function TouristPlaceListScreen() {
    const router = useRouter();
    const { tokenData } = useToken();
    const { fetchAll } = useTouristPlaces();
    ...
    const [allPlaces, setAllPlaces] = useState<TouristPlaceSummary[]>([]);
    const [filteredPlaces, setFilteredPlaces] = useState<TouristPlaceSummary[]>([]);
            ...
            {loading && allPlaces.length === 0 ? (
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <TouristPlaceCard
                            place={item}
                            onPress={() => router.push(`/tourist-place/${item.id}` as any)}
                        />
                    )}
