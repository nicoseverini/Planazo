...
import type { TouristPlaceSummaryResponse } from './tourist-place-shared'
import { Navbar } from './components/Navbar'

export function TouristPlacesPage() {
	const [places, setPlaces] = useState<TouristPlaceSummaryResponse[]>([])
		...
		async function loadPlaces() {
			try {
				...
				if (!response.ok) {
					...
					throw new Error(errorText || 'Could not load tourist places.')
				}

				const data = (await response.json()) as TouristPlaceSummaryResponse[]
				if (isMounted) {
					setPlaces(data)
					setError('')
				}
	...
	return (
		<>
			...
			<main className="auth-card auth-card--xwide">
				...
				{hasAdminAccess && (
					<a className="button button--secondary" href="/create-tourist-place">
			...
			<div className="search-bar">
				<input
					type="text"
					placeholder="Search tourist places by name..."
					value={searchQuery}
			...
			{loading && <div className="message">Loading tourist places...</div>}
					...
					{paginatedPlaces.length > 0 ? (
						paginatedPlaces.map((place) => (
							<li key={place.id} className="plan-item">
								<a href={`/tourist-places/${place.id}`} className="plan-card-link">
						...
						<li className="plan-item">
							<div className="plan-title">No available tourist places.</div>
						</li>
					)}
				</ul>
			)}
