...
export function EditTouristPlacePage({ placeId }: { placeId: number }) {
		...
		async function loadPlace() {
			try {
				...
				if (response.status === 404) {
					throw new Error('The tourist place you are trying to edit was not found.')
				}

				if (!response.ok) {
					const errorText = await response.text()
					throw new Error(errorText || 'Failed to load the tourist place.')
				}

				const data = (await response.json()) as TouristPlaceDetailResponse
				if (isMounted) {
					setForm(toTouristPlaceFormState(data))
	...
	async function handleSubmit(event: FormEvent<HTMLFormElement>) {
		...
		try {
			...
			if (!response.ok) {
				const errorText = await response.text()
				throw new Error(errorText || 'Failed to save the tourist place.')
			}

			window.location.assign(`/tourist-places/${placeId}`)
		} catch (err) {
			setStatus('error')
			setMessage(err instanceof Error ? err.message : 'Unknown error')
		} finally {
		...
		return (
			<>
				...
				<main className="auth-card auth-card--xwide">
					<div className="message">Loading tourist place...</div>
				</main>
			</>
		)
	}
		...
		<>
			...
			<main className="auth-card auth-card--xwide">
				...
				<div>
					...
					<p className="subtitle">Edit the tourist place details and manage its images.</p>
				</div>
				<a className="button button--secondary" href={`/tourist-places/${placeId}`}>
					Back to Details
				</a>
			</div>
